...
public class LogManager : Service
{
	...
	public ILog NewLogFile(SceneType scene = SceneType.Core, LogFileCreateMode CreateMode = LogFileCreateMode.Overwrite)
	{
		string path = Path.Combine(Application.dataPath,scene.ToString());
		path = Path.Combine(path,"Logs");
		path = Path.Combine(path,"Log.txt");
		return new LogFile(path,CreateMode);
	}

	public ILog NewLogFile(string name, SceneType scene = SceneType.Core, LogFileCreateMode CreateMode = LogFileCreateMode.Overwrite)
	{
